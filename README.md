...
Wearly's reasoning is a **structured, inspectable rule layer** documented as a [Claude Skill package](skills/wearly-styling-agent/) (Advanced tier — agentskills.io conventions). Eight rule packs, each cited from the runtime reasoning trail via `[<pack>#R<N>]` slugs in [`rule_refs.py`](rule_refs.py). Every recommendation traces back to a named rule a reviewer can read.
...
Wearly is a working prototype with scope chosen deliberately. The current scope:

- **Inspectable reasoning core.** Recommendations are produced by a structured rule layer documented as a Claude Skill package. Every reasoning line cites a named rule (`<pack>#R<N>`) — see Chapter 04, *"Why the reasoning core is inspectable rules."* This is the auditability + body-positive-language contract; learned layers can ride on top later.
- **Structured-persistence adaptation.** Wearly records explicit signals (wear count, rejections, profile edits) as auditable JSON state. Every learned signal is something a reviewer can open and read — and a future ML layer can use as labeled training data.
...
The **runtime decision engine is auditable by design.** Every reasoning line is generated by inspectable code reading the rule tables — so every line traces back to a cited rule a reviewer can open and read. That's the property that makes the agent's behavior reviewable, the body-positive language contract enforceable, and any future learned layer trainable on labeled outputs.
